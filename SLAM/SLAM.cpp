...
void SLAM::Laser_location()
{
	...
	{
		temp_ransac_match.X[0] = CPSocket::m_pose_data_2.pose_position[0];
		temp_ransac_match.X[1] = 0;
		temp_ransac_match.X[2] = CPSocket::m_pose_data_2.pose_position[1];

		sX = 0;
		sY = -CPSocket::m_pose_data_2.pose_orientation[2];
		sZ = 0;
	}
...
void SLAM::Comparison_Feature(vector<SingleFeature>&l_feature, vector<SingleFeature>&r_feature, vector<PairFeature>&pair_feature)
{
	...
	if (l_feature.size() && r_feature.size())
	{
			...
			//if((isMoveDone&&isHeadingDone) && firststop && stoptime==5) //首停
		{
			...
			else if((Online_Laser_Localization_enable || Offline_Laser_Localization_enable) && !do_onetimes_P3P)
			{
				Laser_location();
			}
...
//----------------------------------------------------------------------------------------------------舊的，刪除不好的特徵點
		...
		for (int k = 0; k < temp1.size(); k++)
		{
			...
			if (1)
			{
				if (temp1[k].unstable_count < 10 && (PhotoCount - temp1[k].appear) < 15)//不穩定次數大於10 或 連續15張未出現  則踢掉特徵 
				{
